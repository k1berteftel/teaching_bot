...
from src.keyboards import (subjects, homework_teacher_builder, confirmed_student,
                           stop_send_homework, access_homework, custom_back_builder, confirmed_teacher, homework_max_builder)
...
async def send_choose_teacher(clb: CallbackQuery, state: FSMContext):
    ...
    if user.role == 'confirmed_student':
        ...
        keyboard = await homework_max_builder(products)
...
async def send_homework(message: Message, state: FSMContext):
    ...
    if user.role == 'confirmed_student':
        ...
        subject: ProductModel = data.get('subject')
        accordance = random.randint(0, 2)
        accuracy = random.randint(0, 2)
        quality = random.randint(0, 2)
        knowledge = random.randint(0, 2)
        independence = random.randint(0, 2)
        balls = accordance + accuracy + quality + knowledge + independence
        text = (f'<b>Учитель Макс проверил вашу домашнюю работу по предмету {subject.subject}</b>\nВот '
                f'результаты проверки домашней работы:\n\n - Полное соответствие заданию:{accordance} (0 - 2 балла)\n'
                f' - Точность: {accuracy} (0 - 2 балла)\n - Качество оформления: {quality} (0 - 2 балла)\n'
                f' - Демонстрация знаний и глубина ответа: {knowledge} (0 - 2 балла)\n'
                f' - Самостоятельность выполнения: {independence} (0 - 2 балла)\n'
                f'Общий балл: {balls} из 10')
        await message.answer(text=text)
        rating = await get_rating(message.from_user.id, subject.subject)
        if rating is None:
            rating = await create_player(message.from_user.id, subject.subject)
        await add_homework(message.from_user.id, balls, subject.subject)
        await state.clear()
        await message.answer(f'Домашнее задание было успешно проверенно. Баллы: {balls} из 10, '
...
async def get_independence(msg: Message, state: FSMContext):
    ...
    await msg.answer(f'Домашнее задание было успешно проверенно. Баллы: {balls} из 10, '
                     f'спасибо вам за проделанную работу')
    await msg.answer('Вы в главном меню', reply_markup=confirmed_teacher)
